...
- 이 게임이 왜 인기인가 · 트렌드 인사이트 · Vision 스크린샷 분석 · 광고 소재 6종 즉시 생성
...
| 기능 | 설명 |
|------|------|
| 실시간 차트 | 글로벌 탑 · 매출 탑 · 캐주얼 탑 · 급상승 4개 탭, 클릭하면 바로 분석 |
| 실시간 데이터 수집 | `google-play-scraper`로 검색 결과 + 차트 순위 동시 수집, 개별 404 무시 |
| 이 게임이 왜 인기인가 | 차트 진입 게임별 개별 분석 — 훅 · 리텐션 · 수익 모델 해부 (Haiku) |
| 4단계 앙상블 파이프라인 | Stage 1 3-way 병렬(Sonnet 트렌드 + Vision + Haiku 왜인기) → Haiku×2 인사이트 앙상블 → Haiku×2 광고 앙상블 → Sonnet 선별 |
| Vision 분석 | 스크린샷 최대 10장을 Claude Vision으로 분석 — 결과가 인사이트·광고 생성에 cascade injection |
| 단계별 스트리밍 | Server-Sent Events(SSE)로 각 파이프라인 단계 완료 시 결과 순차 전송 |
| 광고 소재 6종 | 흥미 · 도전 · 호기심 · FOMO · 단순 · 공감(스토리텔링) 6가지 톤 |
| 차트 순위 뱃지 | 글로벌탑 / 매출탑 / 캐주얼탑 + ▲순위 상승 / 🆕 신규 진입 뱃지 |
...
| 스냅샷 시스템 | 하루 1회 크론(Vercel) + 수동 저장 버튼으로 차트 변화 이력 축적 |
...
google-play-scraper 검색 + 개별 gplay.app() 병렬 수집
        │
        ├─ 차트 DB 조회 → chartRank / chartLabel / rankChange / isNewEntry 뱃지 부여
        └─ 게임 데이터 (GameData[])
                │
                ▼
[/api/analyze]  SSE 스트리밍
        │
        ├─ Stage 1 (3-way 병렬)
        │    ├─ 트렌드 분석가 (Sonnet)       장르 트렌드 · 메카닉 · 키워드 추출
        │    ├─ Vision 분석 (Sonnet Vision)  스크린샷 base64 → UI/메카닉 분석
        │    └─ 왜인기인가 (Haiku)           차트 게임별 개별 분석
        │
        ├─ Stage 2 │ 인사이트 앙상블 (Haiku × 2 병렬)
        │            분석형 A vs 창의형 B → Vision context cascade injection
        │
        ├─ Stage 3 │ 광고 소재 앙상블 (Haiku × 2 병렬)
        │            퍼포먼스형 vs 브랜드형 6종씩 → Vision context cascade injection
        │
        ...
        결과 저장 (Supabase analysis_history)
                ...
                ▼
        결과 페이지 (/result)
        RisingInsightsSection · TrendCard · AdCopyCard × 6 · GameCharts

[/api/cron/snapshot]  매시간 실행 (Vercel Cron)
        └─ 글로벌탑 · 매출탑 · 캐주얼탑 각 30위 → Supabase chart_snapshots

[/api/charts/rising]
        └─ 최신 vs 1시간 이상 전 스냅샷 .lt() 비교 → 급상승 / 신규 진입 게임 탐지
```
...
| AI — 앙상블 | Claude Haiku 4.5 (`claude-haiku-4-5-20251001`) × 5 calls |
| AI — Vision | Claude Sonnet 4 (이미지 base64 멀티모달, Stage 1 병렬) |
...
- Vision 분석은 스크린샷 3장 이상 수집 시 활성화되며, 실패 시 null로 graceful degradation합니다.
